...
$requiredFiles = @(
    @{Name = "ISyncService"; Path = "SyncEkpToCasdoor.Web\Services\ISyncService.cs"}
    @{Name = "SyncService"; Path = "SyncEkpToCasdoor.Web\Services\SyncService.cs"}
    @{Name = "ScheduledSyncService"; Path = "SyncEkpToCasdoor.Web\Services\ScheduledSyncService.cs"}
)
...
$reportPath = Join-Path $PSScriptRoot "test-complete-modules-$(Get-Date -Format 'yyyyMMdd-HHmmss').txt"
@"
