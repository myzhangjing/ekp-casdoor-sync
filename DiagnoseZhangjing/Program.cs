...
// 测试修复后的person_info CTE对张璟的DeptId计算（优先使用fd_parentid）
Console.WriteLine("========== 1. 修复后逻辑 - 张璟的DeptId ==========");
var sql1 = @"
WITH person_info AS (
    SELECT 
        e.fd_id AS PersonId,
...
using (var reader = cmd.ExecuteReader())
{
    if (reader.Read())
    {
        ...
        Console.WriteLine($"DeptId: {deptId.Substring(0, Math.Min(8, deptId.Length))}...");
        ...
        else if (deptName.Contains("技术管理"))
        {
            Console.WriteLine("\n✓✓✓ 成功！张璟正确关联到技术管理部！");
        }
        else
        {
            Console.WriteLine($"\n✗ 问题: 张璟被关联到{deptName}而不是技术管理部");
        }
    }
    else
    {
