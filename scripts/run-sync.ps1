...
$projectDir = Join-Path $repoRoot "SyncEkpToCasdoor"
Set-Location $repoRoot

$dotnet = "C:\Program Files\dotnet\dotnet.exe"
$dll    = Join-Path $projectDir "bin\Release\net8.0\SyncEkpToCasdoor.dll"
$logDir = Join-Path $repoRoot "logs"   # 统一使用仓库根的 logs
...
if (-not (Test-Path $dll)) {
  & $dotnet build "$projectDir\SyncEkpToCasdoor.csproj" -c Release | Out-File -FilePath $logFile -Encoding utf8 -Append
}
