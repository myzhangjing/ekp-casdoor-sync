...
Write-Output "Running direct dotnet: $dotnet $dll --purge-only"
# Run the binary directly (no redirection) so we can see console output live for debugging.
& $dotnet $dll --dry-run

Write-Output "Direct run finished."